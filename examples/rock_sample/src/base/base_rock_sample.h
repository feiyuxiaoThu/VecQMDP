...
protected:
	...
	int start_rock_bits_; // -1 = random; >= 0 = fixed good/bad bitmask
...
public:
	...
	/** Fix which rocks are good at the start.
	 *  @param bits  Bitmask: bit i = 1 means rock i is good.
	 *               Pass -1 (default) to restore random sampling. */
	void SetStartRockBits(int bits) { start_rock_bits_ = bits; }
	int  GetStartRockBits() const   { return start_rock_bits_; }
	...
	/**
	 * Rich ASCII visualization of the current state and belief.
	 *
	 * Grid symbols:
	 *   @  = agent
	 *   $  = good rock (true state)
	 *   x  = bad / sampled rock (true state)
	 *   .  = empty cell
	 *
	 * @param state          True world state (for agent pos + rock quality).
	 * @param rock_good_prob P(rock_i is good) from the particle belief,
	 *                       one entry per rock.  May be empty to skip belief.
	 * @param out            Output stream (default: stdout).
	 */
	void PrintStateViz(const State& state,
	                   const std::vector<double>& rock_good_prob,
