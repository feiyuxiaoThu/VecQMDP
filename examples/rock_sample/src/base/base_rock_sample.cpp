...
void BaseRockSample::Init_15_15() {
	// Equivalent to RockSample_11_11.pomdp(x)
	cout << "Using special layout for rocksample(15, 15)" << endl;

	Coord rocks[] = { Coord(14, 14), Coord(14, 2), Coord(3, 3), Coord(8, 14), Coord(3, 5), 
		Coord(12, 7), Coord(1, 11), Coord(12, 6), Coord(11, 7), Coord(12, 9), Coord(11, 4), 
		Coord(2, 6), Coord(13, 8), Coord(5, 4), Coord(7, 13) };
...
State* BaseRockSample::CreateStartState(string type) const {
	...
	if (start_rock_bits_ >= 0) {
		state |= (start_rock_bits_ & ((1 << num_rocks_) - 1));
	} else {
		for (int i = 0; i < num_rocks_; i++) {
...
public:
	...
	double Value(const State& state) const {
		// std::cout << "RockSampleMDPParticleUpperBound" << std::endl;
...
public:
	...
	double Value(const State& state) const {
		// std::cout << "RockSampleApproxParticleUpperBound" << std::endl;
		double value = 0;
		double discount = 1.0;
...
ScenarioUpperBound* BaseRockSample::CreateScenarioUpperBound(string name,
	...
	std::cout << "Creating scenario upper bound: " << name << std::endl;
	std::cout << "Particle upper bound name: " << particle_bound_name << std::endl;
	name = "TRIVIAL";
...
public:
	...
	ValuedAction Value(const vector<State*>& particles, RandomStreams& streams,
		...
		// std::cout << "RockSampleEastScen、arioLowerBound" << std::endl;
...
ScenarioLowerBound* BaseRockSample::CreateScenarioLowerBound(string name, string
	...
	std::cout << "Creating scenario lower bound: " << name << std::endl;
	std::cout << "Particle lower bound name: " << particle_bound_name << std::endl;
...
void BaseRockSample::PrintStateViz(const State& state,
                                   const vector<double>& rock_good_prob,
                                   ostream& out) const {
	// ---- Grid ----
	const int W = size_ * 3 + 1;   // column width for the border line
	out << "\n+";
	for (int x = 0; x < size_; ++x) out << "---";
	out << "-+\n";
	...
	for (int y = size_ - 1; y >= 0; --y) {
		...
		for (int x = 0; x < size_; ++x) {
			Coord pos(x, y);
			if (GetRobPos(&state) == pos) {
				out << " @ ";
			} else {
				int rock = grid_(pos);
				if (rock >= 0) {
					bool good = GetRock(&state, rock);
					// rock index suffix for disambiguation when many rocks
					out << (good ? " $ " : " x ");
				} else {
					out << " . ";
				}
	...
	for (int x = 0; x < size_; ++x) out << "---";
	out << "-+\n";
	out << "  Legend:  @=agent  $=good rock  x=bad/sampled rock  .=empty\n";

	// ---- Rock index overlay (show which cell holds which rock number) ----
	out << "\nRock positions:";
	for (int r = 0; r < num_rocks_; ++r)
		out << "  R" << r << "@(" << rock_pos_[r].x << "," << rock_pos_[r].y << ")";
	out << "\n";

	// ---- Belief bar chart ----
	if (!rock_good_prob.empty()) {
		const int BAR = 16;
		out << "\nBelief  P(good):\n";
		for (int r = 0; r < num_rocks_; ++r) {
			double p = (r < static_cast<int>(rock_good_prob.size()))
			           ? rock_good_prob[r] : 0.5;
			bool true_good = GetRock(&state, r);

			int filled = static_cast<int>(p * BAR + 0.5);
			filled = filled < 0 ? 0 : (filled > BAR ? BAR : filled);

			out << "  R" << r
			    << " [" << (true_good ? "GOOD" : " BAD") << "]"
			    << "  p=" << fixed << setprecision(2) << p << "  |";
			for (int i = 0; i < BAR; ++i)
				out << (i < filled ? '#' : '.');
			out << "|\n";
		}
