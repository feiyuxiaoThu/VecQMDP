...
class QCNetMapEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        # Discrete feature embedding layers - encode categorical information
        if dataset == 'argoverse_v2':
            # Map-point type embedding
            self.type_pt_emb = nn.Embedding(17, hidden_dim)  # 17 different map-point types
            self.side_pt_emb = nn.Embedding(3, hidden_dim)   # 3 point positions (left / center / right)
            # Polygon type embedding
            self.type_pl_emb = nn.Embedding(4, hidden_dim)   # 4 different polygon types
            self.int_pl_emb = nn.Embedding(3, hidden_dim)    # Intersection flag (3 values)
        elif dataset == 'nuplan':
            # Map-point type embedding
            self.type_pt_emb = nn.Embedding(3, hidden_dim)  # 3 map-point types (left / center / right)
            self.side_pt_emb = nn.Embedding(3, hidden_dim)   # 3 point positions (left / center / right)
            # Polygon type embedding
            self.type_pl_emb = nn.Embedding(3, hidden_dim)   # 3 polygon types (lane, lane_connector, crosswalk)
            self.int_pl_emb = nn.Embedding(2, hidden_dim)    # Intersection flag (2 values)
        else:
            raise ValueError('{} is not a valid dataset'.format(dataset))
    ...
    def forward(self, data: HeteroData) -> Dict[str, torch.Tensor]:
        ...
        # Handle empty polygon case: radius_graph cannot process empty tensors
        num_polygons = pos_pl.size(0)
        if num_polygons == 0:
            edge_index_pl2pl_radius = torch.empty(2, 0, dtype=torch.long, device=pos_pl.device)
        else:
