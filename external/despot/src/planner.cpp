...
void Planner::PlanningLoop(Solver*& solver, World* world, Logger* logger) {
	for (int i = 0; i < Globals::config.sim_len; i++) {
		...
		const auto plan_time = std::chrono::duration<double, std::milli>(plan_end - plan_start).count();
		std::cout << "[PlanningLoop] Time spent in RunStep(): " << plan_time 
			 	  << "ms, simulation throughput " << simulation_count_ / plan_time << "#/ms" << std::endl;
		simulation_count_ = 0;
