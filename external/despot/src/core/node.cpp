...
int VNode::GetMaxDepth() {
    int max_child_depth = 0;
    
    const vector<QNode*>& qnodes = children();
    for (int a = 0; a < qnodes.size(); a++) {
        QNode* qnode = qnodes[a];
        map<OBS_TYPE, VNode*>& vnodes = qnode->children();
        
        for (map<OBS_TYPE, VNode*>::iterator it = vnodes.begin(); it != vnodes.end(); it++) {
            VNode* child_vnode = it->second;
            if (child_vnode) {
                int d = child_vnode->GetMaxDepth() + 1;
...
void VNode::PrintTree(int depth, ostream& os) {
	...
	int real_max_depth = GetMaxDepth(); 
	os << "---------------------------------" << endl;
	os << "Search Tree Maximum Depth: " << real_max_depth << endl;
}
