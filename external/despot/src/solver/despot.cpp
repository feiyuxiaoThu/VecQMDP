...
// Per-cycle timing accumulators for Step() and lower-bound Value() (rollout).
// Reset and printed in Search() after each planning cycle.
static double  s_step_total_us      = 0.0;
static int64_t s_step_call_count    = 0;
static double  s_rollout_total_us   = 0.0;
static int64_t s_rollout_call_count = 0;
...
void DESPOT::InitLowerBound(VNode* vnode, ScenarioLowerBound* lower_bound,
	...
	// const auto t0_rollout = std::chrono::steady_clock::now();
	ValuedAction move = lower_bound->Value(vnode->particles(), streams, history);
	// s_rollout_total_us += std::chrono::duration<double, std::micro>(
	// 	std::chrono::steady_clock::now() - t0_rollout).count();
	// s_rollout_call_count += vnode->particles().size();
...
ValuedAction DESPOT::Search() {
	...
	root_->PrintTree(0, cout);

	// std::cout << "simulation count: " << simulation_count_ << std::endl;
	// std::cout << "througout: " << simulation_count_ / ((get_time_second() - start) * 1000.0) << " #/ms" << std::endl;
	// simulation_count_ = 0;
	...
	// 		(s_step_call_count > 0)
	// 		? s_step_total_us / static_cast<double>(s_step_call_count)
	// 		: 0.0;
	// 	const double avg_rollout_us =
	// 		(s_rollout_call_count > 0)
	// 		? s_rollout_total_us / static_cast<double>(s_rollout_call_count)
	// 		: 0.0;
	// 	std::cout << std::fixed << std::setprecision(3)
	// 		<< "[DESPOT timing]  Step: "
	// 		<< avg_step_us << " µs/call"
	// 		<< " (" << s_step_call_count << " calls)"
	// 		<< " | Rollout(Value): "
	// 		<< avg_rollout_us << " µs/call"
	// 		<< " (" << s_rollout_call_count << " calls)\n";
	// 	s_step_total_us      = 0.0; s_step_call_count    = 0;
	// 	s_rollout_total_us   = 0.0; s_rollout_call_count = 0;
	// }
...
void DESPOT::Expand(VNode* vnode,
	...
	for (ACT_TYPE action = 0; action < model->NumActions(); action++) {
		...
		const vector<State *> &particles = vnode->particles();
		simulation_count_ += particles.size();
	}
...
void DESPOT::Expand(QNode* qnode, ScenarioLowerBound* lb,
	...
	for (int i = 0; i < particles.size(); i++) 
	{
		...
		// const auto t0_step = std::chrono::steady_clock::now();
		bool terminal = model->Step(*copy, streams.Entry(copy->scenario_id),
			qnode->edge(), reward, obs);
		// s_step_total_us += std::chrono::duration<double, std::micro>(
		// 	std::chrono::steady_clock::now() - t0_step).count();
		// ++s_step_call_count;
