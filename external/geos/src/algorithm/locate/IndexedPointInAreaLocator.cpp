...
std::vector<IndexedPointInAreaLocator::SegmentView>
IndexedPointInAreaLocator::collectSegments(const double x, const double y)
{
    if (index == nullptr) {
        buildIndex(areaGeom);
    }
    ...
    index->query(y, y, [&segments](const SegmentView& ls) {
        segments.emplace_back(ls);
    });
    return segments;
}

void IndexedPointInAreaLocator::updateIndex()
{
    if (index == nullptr) {
        buildIndex(areaGeom);
    }
