...
namespace vamp
{
    ...
    struct VectorInterface
    {
        ...
        // Creates a vector with elements [start, start+1, start+2, ...] 
        // filling each SIMD vector in sequence
        inline static constexpr auto iota(typename S::ScalarT start) noexcept -> D
        {
            return D(make_iota_array(start, std::make_index_sequence<num_vectors>()));
        }

        template <std::size_t n_v = num_vectors, typename = std::enable_if_t<n_v == 1, bool>>
        inline static constexpr auto create_tail_mask(int remainder) noexcept -> D
        {
            return D({S::template create_tail_mask<0>(remainder)});
        }
        ...
        inline static constexpr auto load_contiguous_unaligned(const typename S::ScalarT *base, typename S::ScalarT offset) noexcept -> D
        {
            return D(load_contiguous_impl_unaligned(base + static_cast<std::size_t>(offset), std::make_index_sequence<num_vectors>()));
        }

        inline static constexpr auto load_contiguous(const typename S::ScalarT *base, typename S::ScalarT offset) noexcept -> D
        {
            return D(load_contiguous_impl(base + static_cast<std::size_t>(offset), std::make_index_sequence<num_vectors>()));
        }

        template <std::size_t... I>
        inline static constexpr auto
        load_contiguous_impl_unaligned(const typename S::ScalarT *const base, std::index_sequence<I...>) noexcept -> DataT
        {
            return {(S::template load_unaligned<0>(base + I * S::VectorWidth))...};
        }

        template <std::size_t... I>
        inline static constexpr auto
        load_contiguous_impl(const typename S::ScalarT *const base, std::index_sequence<I...>) noexcept -> DataT
        {
            return {(S::template load<0>(base + I * S::VectorWidth))...};
        }
        template <
            typename IndexT,
            typename MaskT,
        ...
        template <typename IndexT, typename = std::enable_if_t<same_num_scalars<D, IndexT>::value>>
        inline constexpr void scatter(typename S::ScalarT *base, const IndexT &idxs) const noexcept
        {
            scatter_helper(base, idxs.data, std::make_index_sequence<num_vectors>());
        }
        ...
        template <typename T, typename allow_types<D>::template check<T> = true>
        inline constexpr auto mod(T o) const noexcept -> D
        {
            return mod(o.data);
        }

        template <typename T, typename allow_types<DataT, typename S::VectorT>::template check<T> = true>
        inline constexpr auto mod(T other) const noexcept -> D
        {
            return D(apply<S::template mod<0>>(d()->data, other));
        }
        ...
        template <typename T, typename allow_types<D>::template check<T> = true>
        inline constexpr auto clamp(T lower, T upper) const noexcept -> D
        {
            return clamp(lower.data, upper.data);
        }

        template <typename T, typename allow_types<DataT>::template check<T> = true>
        inline constexpr auto clamp(T lower, T upper) const noexcept -> D
        {
            return D(apply<S::template clamp<0>>(d()->data, lower, upper));
        }
        ...
        inline constexpr auto hmax() const noexcept -> typename S::ScalarT
        {
            ...
            {
                return S::template hmax<0>(d()->data[0]);
            }
            else
            {
                typename S::ScalarT max_val = S::extract(d()->data[0], 0);
                for (std::size_t i = 0; i < num_vectors; ++i)
                {
                    for (std::size_t j = 0; j < S::VectorWidth; ++j)
                    {
                        typename S::ScalarT val = S::extract(d()->data[i], j);
                        if (val > max_val)
                        {
                            max_val = val;
                        }
        ...
        inline constexpr auto square() const noexcept -> D
        {
            return D(apply<S::template mul<0>>(d()->data, d()->data));
        }
        
        inline constexpr auto sign() const noexcept -> D
        {
            return D(apply<S::template sign<0>>(d()->data));
        }
        
        template <int exp = 2>
        inline constexpr auto pow(typename S::ScalarT exponent = static_cast<typename S::ScalarT>(exp)) const noexcept -> D
        {
            if (exponent == static_cast<typename S::ScalarT>(2)) {
                return square();
            }
            else if (exponent == static_cast<typename S::ScalarT>(4)) {
                auto squared = square();
                return squared.square();
            }
            
            return D(apply<S::template pow_vector<0>>(d()->data, exponent));
        }
        ...
        template <
            ...
            typename =
                std::enable_if_t<std::is_same_v<ScalarT, float> or std::is_same_v<ScalarT, double>, bool>>
        inline constexpr auto tan() const noexcept -> D
        {
            return d()->sin() / d()->cos();
        }
        ...
        template <
            ...
            typename = 
                std::enable_if_t<std::is_same_v<ScalarT, float> or std::is_same_v<ScalarT, double>, bool>>
        inline constexpr void sincos(D& out_sin, D& out_cos) const noexcept
        {
            [&]<std::size_t... I>(std::index_sequence<I...>)
            {
                (S::sincos(d()->data[I], out_sin.data[I], out_cos.data[I]), ...);
            }(std::make_index_sequence<num_vectors>{});
        }
        ...
        template <typename OtherT, typename std::enable_if_t<not std::is_same_v<OtherT, D>, bool> = true>
        inline constexpr auto convert() const noexcept -> OtherT
        {
            return OtherT{apply<S::template convert<typename OtherT::S::VectorT>>(d()->data)};
        }
        ...
        template <typename MaskT, typename = std::enable_if_t<same_num_scalars<D, MaskT>::value>>
        inline static constexpr auto select(const MaskT &mask, const D &when_true, const D &when_false) noexcept -> D
        {
            return D(apply<S::template select<0>>(mask.data, when_true.data, when_false.data));
        }

        template <typename = void>
        inline static constexpr auto select(const typename S::VectorT &mask, const D &when_true, const D &when_false) noexcept -> D
        {
            return D(apply<S::template select<0>>(mask, when_true.data, when_false.data));
        }
        ...
        inline constexpr auto hmin() const noexcept -> typename S::ScalarT
        {
            if constexpr (num_vectors == 1)
            {
                return S::hmin(d()->data[0]);
            }
            else
            {
                typename S::VectorT min_vector = d()->data[0]; 

                for (std::size_t i = 1; i < num_vectors; ++i)
                {
                    min_vector = S::min(min_vector, d()->data[i]);
                }
                
                return S::hmin(min_vector);
            }
        }

        template <typename T, typename allow_types<D>::template check<T> = true>
        inline constexpr auto atan2_approx(T o) const noexcept -> D
        {
            return atan2_approx(o.data);
        }

        template <typename T, typename allow_types<DataT, typename S::VectorT>::template check<T> = true>
        inline constexpr auto atan2_approx(T other) const noexcept -> D
        {
            return D(apply<S::template atan2_approx<0>>(d()->data, other));
        }

        inline constexpr auto atan2_approx(typename S::ScalarT other) const noexcept -> D
        {
            return D(apply<S::template atan2_approx<0>>(d()->data, broadcast_scalar(other)));
        }
    protected:
        template <std::size_t... I, typename IndexDataT>
        inline constexpr void scatter_helper(typename S::ScalarT *base, const IndexDataT &idx_data, std::index_sequence<I...>) const noexcept
        {
            (..., S::template scatter<void>(std::get<I>(d()->data), base, std::get<I>(idx_data)));
        }
        ...
        template <std::size_t... I>
        inline constexpr auto
        broadcast_array_impl(const std::array<typename S::ScalarT, num_rows>& scalar_data, std::index_sequence<I...>) noexcept -> DataT
        {
            return {(S::template constant<0>(scalar_data[I / Sig::num_vectors_per_row]))...};
        }

        inline constexpr void
        broadcast_array(std::array<typename S::ScalarT, num_rows> scalar_data) noexcept
        {
            d()->data = broadcast_array_impl(scalar_data, std::make_index_sequence<num_vectors>());
        }

        template <std::size_t... I>
        inline constexpr auto
        broadcast_vector_impl(const std::vector<typename S::ScalarT>& scalar_data, std::index_sequence<I...>) noexcept -> DataT
        {
            return {(S::template constant<0>(scalar_data[I / Sig::num_vectors_per_row]))...};
        }
        ...
        inline void broadcast_vector(std::vector<typename S::ScalarT> scalar_data) noexcept
        {
            ...
            d()->data = broadcast_vector_impl(scalar_data, std::make_index_sequence<num_vectors>());
        }
        ...
        template <std::size_t... I>
        inline static constexpr auto
        make_iota_array(typename S::ScalarT start, std::index_sequence<I...> /* indices */) noexcept -> DataT
        {
            return {(S::template iota<0>(start + static_cast<typename S::ScalarT>(I * S::VectorWidth)))...};
        }
    };
                        ...
                        VectorSignature<SimdT, num_rows, num_scalars_per_row>>
    {
            ...
            typename S::ScalarT
        {
            ...
            auto vec_idx = row_idx * num_vectors_per_row + col_idx / S::VectorWidth;
            auto elem_idx = col_idx % S::VectorWidth;
            return S::extract(data[vec_idx], elem_idx);
        }
