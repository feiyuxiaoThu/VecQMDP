...
namespace vamp
{
    namespace
    {
        ...
        // Void version for operations with no return value (like scatter)
        template <auto fn, typename DataT, typename IndexT, typename... FixedArgT>
        struct applicator_void
        {
            template <std::size_t... I>
            inline static void
            run_indexed(std::index_sequence<I...>, const DataT data, const IndexT idx_data, FixedArgT... fixed_args) noexcept
            {
                // fn expects: (VectorT value, ScalarT* base, VectorT index)
                // fixed_args contains base pointer
                (..., fn(std::get<I>(data), fixed_args..., std::get<I>(idx_data)));
            }
        };

        template <auto fn, typename DataT, typename IndexT, typename... FixedArgT>
        inline constexpr void apply_indexed_void(DataT data, IndexT idx_data, FixedArgT... fixed_args) noexcept
        {
            applicator_void<fn, DataT, IndexT, FixedArgT...>::run_indexed(
                std::make_index_sequence<data_size<DataT>()>(), data, idx_data, fixed_args...);
        }
